...
class PI05RolloutRequest(PI05Request):
    """Fully specified trajectory execution request.

    The canonical model inputs are inherited from :class:`PI05Request`.
    ``noise``, ``step_noise``, and ``sigmas`` completely determine the sampled
    trajectory. Policy-level choices such as denoise-step selection, action
    cropping, and log-probability reduction belong to the caller.
    """
...
class PI05Scheduler(Scheduler):
    ...
    def rollout_step(self, request: PI05RolloutRequest) -> PI05RolloutOutput:
        """Execute a fully specified stochastic-flow trajectory.
        ...
        random inputs explicitly, and returns raw trajectory, probability,
        and optional value outputs for the caller to interpret.
        """
        self._validate(request)
        actual_B = int(request.pixel_values.shape[0])
        request_noise, request_step_noise, request_sigmas = self._validate_rollout(
            request, actual_B
        )
        ...
        noise[:actual_B] = request_noise.to(device=self.device, dtype=torch.float32)
        ...
        step_noise[:actual_B] = request_step_noise.to(
    ...
    def _build_layout(
        ...
        # The expert's joint pass sees each sample's real prefix plus its own
        # whole action chunk.
    ...
    def _validate_rollout(
        self, req: PI05RolloutRequest, actual_B: int
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Validate the fully specified rollout execution tensors."""
        cfg = self.cfg
        if req.noise is None:
            raise ValueError("rollout execution requires caller-provided noise.")
        if req.step_noise is None:
            raise ValueError("rollout execution requires caller-provided step_noise.")
        ...
        if req.step_noise.shape != expected_step_noise:
            raise ValueError(
        ...
        return req.noise, req.step_noise, req.sigmas
