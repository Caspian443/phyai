...
def test_weight_load_session_applies_streamed_tensors(fake_mesh):
    fake_mesh(sizes={"tp": 1})
    _init_dispatcher()
    layer = L.ReplicatedLinear(
        in_features=4,
    ...
    session.load({"mod.fc.weight": src_w})
    session.load({"mod.fc.bias": src_b})
...
def test_weight_load_session_strict_missing(fake_mesh):
    fake_mesh(sizes={"tp": 1})
    _init_dispatcher()
    layer = L.ReplicatedLinear(
        in_features=4,
    ...
    session.load({"mod.fc.weight": torch.randn(8, 4)})
