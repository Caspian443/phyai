...
<ModelCard
  ...
  rows={{
    ...
    "Tokenizer 长度": "200",
    "运行入口": <code className="px-2 py-0.5 rounded bg-[#003399]/10 dark:bg-[#60A5FA]/15 text-[#003399] dark:text-[#60A5FA] text-xs font-mono">PI05Scheduler</code>,
    "参数精度": "bf16",
...
本页文档专注于 `ws1` 即 `world_size=1`,单 rank、不走分布式。本页全部内容都围绕这套单卡配置。 `entry` 是 `PI05Scheduler`。
...
![PhyAI Engine ↔ Scheduler ↔ 3 Runners 生命周期](/images/models/pi05/engine_lifecycle.svg)
...
外部 RL controller 可以读取 diffusion trajectory，也可以替换内存中的 Actor 权重，无需重建 engine。使用完整 checkpoint 初始化 engine，并在构造 `PI05Args` 时设置 `capture_rollout=True`；训练期间的更新可以只包含 controller 选择的参数。
...
每个 `weight_batch` 都是 HuggingFace 参数名到 tensor 的映射。每次完成更新后都会刷新依赖权重的 scheduler 状态，并重新捕获 rollout graph。`rollout_step` 接收 `PI05RolloutRequest`，返回 `PI05RolloutOutput`，其中包含 action、denoising chain、逐 transition log probability 和可选的 value。请求必须提供 `sigmas`；`noise` 和 `step_noise` 是可选的确定性输入，未提供时由 runtime 内部采样。

RLinf BF16 集成使用可选的兼容策略 `examples/configs/kernel_policies/pi05/rlinf_bf16.yaml`。应通过 controller 配置或 `PHYAI_KERNEL_CONFIG` 显式选择；PI0.5 默认不会自动加载该策略。

<Warning>
  部分权重写入后如果更新失败，不要继续用这个 `Engine` 推理。此时参数可能混合了两个版本，`Engine` 会把实例标记为不可用。终止事务后重新创建 engine。
</Warning>

# 当前限制

- 仅支持单卡。Tensor parallel、continuous batching、preemption 都不在 `PI05Scheduler` 的范围内。
