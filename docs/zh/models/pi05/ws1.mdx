...
外部 RL controller 可以读取 diffusion trajectory，也可以替换内存中的 Actor 权重，无需重建 engine。构造 `PI05Args` 时设置 `capture_rollout=True`。如果初始权重由 Actor 提供，还要设置 `defer_scheduler_setup=True`。第一次更新必须提供完整模型权重；完成 scheduler setup 和 CUDA Graph capture 后，后续更新可以只包含部分权重。
...
每个 `weight_batch` 都是 HuggingFace 参数名到 tensor 的映射。第一次更新会检查 missing 和 unexpected key，后续更新可以只包含部分权重。每次完成更新后都会刷新依赖权重的 scheduler 状态，并重新捕获 rollout graph。`rollout_step` 接收 `PI05RolloutRequest`，返回 `PI05RolloutOutput`，其中包含 action、denoising chain、逐 transition log probability 和可选的 value。
