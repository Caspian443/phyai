...
An external RL controller can request diffusion trajectories and replace the in-memory actor weights without rebuilding the engine. Construct `PI05Args` with `capture_rollout=True`. If the actor provides the initial weights, also set `defer_scheduler_setup=True`. The first update must provide the complete model state; after scheduler setup and CUDA Graph capture, later updates may be partial.
...
Each `weight_batch` maps HuggingFace parameter names to tensors. The initial update validates missing and unexpected keys; subsequent updates may be partial. Every completed update refreshes weight-dependent scheduler state and recaptures the rollout graph. `rollout_step` accepts `PI05RolloutRequest` and returns `PI05RolloutOutput` with actions, the denoising chain, per-transition log probabilities, and optional values.
